...
public class SchoolDAO extends DBconnect{
	...
	public void saveData(String id, int schoolNum) {
		// id변수는  로그인한 아이디 이고 ,  schoolNum은 선택한 학교 번호(id)이다.
		// insert, delete, update 는 executeUpdate 메서드를 사용하고
		// 결과는 없기에 ResultSet으로 안받아도 된다. 
		// 내가 선택한 학교번호가 테이블에 저장 될수 있게 saveData 메서드 내용을 
		// 완성 하세요.
		// 1분단(명준), 2분단(선우), 3분단(건태) - 경쟁 !  
		// 어느 분단에서 가장 먼저 전부 만드는지  !! 
		// 가장 늦게 만드는 분단은  벌칙?벌점? 암튼 머있음!! 
		
		String sql = "insert into my_school(member_id, school_id) "+
		           " values('"+id+"', "+schoolNum+")";
